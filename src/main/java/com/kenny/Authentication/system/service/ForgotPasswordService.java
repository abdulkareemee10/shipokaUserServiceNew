...
import com.kenny.Authentication.system.repository.ForgotPasswordRepository;
...
public class ForgotPasswordService {
    ...
    public Response changePassword(ForgotPasswordRequest forgotPasswordRequest, String email) {
        ...
        if(forgotPasswordRequest.getPassword().equals(forgotPasswordRequest.getConfirmPassword())) {
            ...
            forgotPassword.setEmailAddress(forgotPasswordRequest.getEmail());
            forgotPassword.setOtp(String.valueOf(otp));
            forgotPassword.setTimeCreated(LocalDateTime.now());
            forgotPassword.setTimeExpired(LocalDateTime.now().plusMinutes(30));
            forgotPassword.setUserId(forgotPassword.getUserId());
            forgotPasswordRepository.save(forgotPassword);


            return Response.builder().responseMessage("SUCCESS").build();
        }return Response.builder().statusCode("400").responseMessage(" password does not match ").build();
    }
